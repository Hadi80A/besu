...
import static org.hyperledger.besu.consensus.pos.statemachine.PosBlockHeightManager.MessageAge.CURRENT_ROUND;
import static org.hyperledger.besu.consensus.pos.statemachine.PosBlockHeightManager.MessageAge.FUTURE_ROUND;
import static org.hyperledger.besu.consensus.pos.statemachine.PosBlockHeightManager.MessageAge.PRIOR_ROUND;
    ...
    private void logValidatorChanges(final PosRound posRound) {
        if (posRound.getRoundIdentifier().getRoundNumber() == 0) {
            final Collection<Address> previousValidators =
                    MessageValidatorFactory.getValidatorsForBlock(posRound.protocolContext, parentHeader);
            final Collection<Address> validatorsForHeight =
                    MessageValidatorFactory.getValidatorsAfterBlock(posRound.protocolContext, parentHeader);
