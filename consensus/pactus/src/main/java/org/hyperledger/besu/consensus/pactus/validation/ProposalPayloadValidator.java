...
public class ProposalPayloadValidator {
  ...
  private final PactusBlockValidator blockValidator;
  ...
  public ProposalPayloadValidator(
      ...
      final PactusBlockValidator blockValidator) {
   ...
   * @return the boolean
   */
  public boolean validate(final SignedData<ProposePayload> signedPayload) {
    ...
    final ProposePayload payload = signedPayload.getPayload();
    ...
    final PactusBlock block = payload.getPactusBlock();
    if (!validateBlock(block)) {
      return false;
    }

    if (block.getBesuBlock().getHeader().getNumber() != payload.getRoundIdentifier().getSequenceNumber()) {
  ...
  private boolean validateBlock(final PactusBlock block) {
