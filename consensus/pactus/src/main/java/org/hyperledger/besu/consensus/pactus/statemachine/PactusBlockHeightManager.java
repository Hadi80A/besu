 ...
 */
public class PactusBlockHeightManager implements BasePactusBlockHeightManager  {

  private static final Logger LOG = LoggerFactory.getLogger(PactusBlockHeightManager.class);
  ...
  private Optional<PactusRound> currentRound = Optional.empty();
  ...
  /**
   * Instantiates a new Pactus block height manager.
   *
   * @param parentHeader the parent header
   * @param finalState the final state
   * @param pactusRoundFactory the pactus round factory
  ...
  /**
   * Instantiates a new Pactus block height manager. Secondary constructor with early round change
   * option.
   *
   * @param parentHeader the parent header
   * @param finalState the final state
...
//    startNewRound(0);
//
//    final PactusRound pactusRound = currentRound.get();
//
//    logValidatorChanges(pactusRound);
//
//    if (roundIdentifier.equals(pactusRound.getRoundIdentifier())) {
//      buildBlockAndMaybePropose(roundIdentifier, pactusRound);
//    } else {
//      LOG.trace(
...
//            finalState.isLocalNodeProposerForRound(pactusRound.getRoundIdentifier());
//
//    if (!isProposer) {
//      // nothing to do here...
...
//    final PactusBlock block = pactusRound.createBlock(headerTimeStampSeconds);
//    if (!block.isEmpty()) {
//      LOG.trace(
//              "Block is not empty and this node is a proposer so it will send a proposal: "
//                      + roundIdentifier);
//      pactusRound.updateStateWithProposalAndTransmit(block);
//    } else {
...
//    }
   ...
   *
   * @param pactusRound The current round
   */
  private void logValidatorChanges(final PactusRound pactusRound) {
//    if (pactusRound.getRoundIdentifier().getRoundNumber() == 0) {
...
//    if (!expire.getView().equals(pactusRound.getRoundIdentifier())) {
//      LOG.trace(
...
//    final Optional<PreparedCertificate> preparedCertificate = currentRound.get().constructPreparedCertificate();
//
...
//      return;
//    }
//    PactusRound pactusRoundNew = currentRound.get();
//
//    try {
//      final RoundChange localRoundChange =
//              messageFactory.createRoundChange(
//                      pactusRoundNew.getRoundIdentifier(), latestPreparedCertificate);
//
...
//    }
//
//    transmitter.multicastRoundChange(pactusRoundNew.getRoundIdentifier(), latestPreparedCertificate);
  }
...
//            RoundState::addPrepareMessage);
  ...
  public void handlePreCommitPayload(final PreCommit precommit) {
...
//            RoundState::addCommitMessage);
  ...
  public void handleRoundChangePayload(final RoundChange message) {
    ...
    Optional<Collection<RoundChange>> result=Optional.empty();
...
//              roundChangeManager.futureRCQuorumReceived(pactusRound.getRoundIdentifier());
...
//    roundChangeManager.discardRoundsPriorTo(currentRound.get().getRoundIdentifier());
  ...
  public PactusBlockHeader getParentBlockHeader() {
    return parentHeader;
  }
