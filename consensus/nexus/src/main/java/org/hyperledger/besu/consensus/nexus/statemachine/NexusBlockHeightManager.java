 ...
 */
public class NexusBlockHeightManager implements BaseNexusBlockHeightManager {
    ...
    private NexusExtraData posExtraData;
...
//    final Map<Address, ViewChange> receivedMessages = Maps.newLinkedHashMap();
    ...
    /**
     * Instantiates a new Nexus block height manager.
     *
     * @param parentHeader    the parent header
     * @param finalState      the final state
     * @param posRoundFactory the nexus round factory
...
//    }
    ...
    //    private void logValidatorChanges(final NexusRound posRound) {
...
//        LOG.info(
//                "NEXUS Validator list change. Previous chain height {}: {}. Current chain height {}: {}.",
...
//    }
//
//    NexusRound posRound = currentRound.get();
...
//    }
    ...
    public boolean checkValidState(int msgCode) {
        ...
        if(msgCode==NexusMessage.SELECT_LEADER.getCode() && currentStateCode == NexusMessage.VOTE.getCode() ) {
            return true;
        }
...
//        LOG.debug("finalState.getQuorum() {}",finalState.getQuorum());
    ...
    private NexusExtraData readNexusData() {
        var extraData = blockchain.getChainHeadBlock().getHeader().getExtraData();
        return new NexusExtraDataCodec().decodeNexusData(extraData);
    }
...
//      handleViewChangePayload(localViewChangeMessage);
    ...
    private RoundState getRoundState() {
        return currentRound.map(NexusRound::getRoundState).orElse(null);
    }
        ...
        if(determineAgeOfPayload(msg.getRoundIdentifier().getRoundNumber())==MessageAge.CURRENT_ROUND) {
            ...
            if (checkThreshold(getRoundState().getSelectLeaderMessages(),false) && proposerSelector.getCurrentProposer().isEmpty()) {
                ...
                if (proposerSelector.isLocalProposer() && currentRound.isPresent() && Objects.nonNull(selected)) {
                    ...
                    getRoundState().setCurrentState(NexusMessage.VOTE);
                    currentRound.get().createProposalAndTransmit(clock, selected.getSignedPayload().getPayload().getProof()
                    );
                }else {
                    getRoundState().setCurrentState(NexusMessage.PROPOSE);
                }
            }
            else {
...
//                    proposerSelector.getSeedAtRound(roundNumber.getRoundNumber()-1, blockchain.getChainHeadHash(), getRoundState().getHeight()));
    ...
    private void sendVote(final NexusBlock block) {
    ...
    private Bls.Signature getBlsSignature(NexusBlock block) {
        String context= NexusMessage.VOTE.getCode() +"||"+ block.getHeader().getRoundIdentifier().getRoundNumber()
...
//aggre
    ...
    private boolean validateBlsSignature(Bls.Signature blsSignature, VotePayload votePayload, Address author) {
        String context = NexusMessage.VOTE.getCode() + "||" + votePayload.getRoundIdentifier().getRoundNumber()
    ...
    private boolean validateHeightAndRound(NexusPayload posPayload) {
    ...
    private boolean validateBlockHash(Hash blockHash) {
        ...
        NexusBlock block = getRoundState().getProposeMessage().getSignedPayload().getPayload().getProposedBlock();
        return blockHash.equals(block.getHash());
    }

    private void sendCommit(final NexusBlock block) {
        ...
        getRoundState().setCurrentState(NexusMessage.COMMIT);
        try {
            if (currentRound.isPresent()) {
    ...
    private QuorumCertificate getQuorumCertificate(NexusBlock block, byte[] aggregate) {
        if(currentRound.isPresent()) {
            ...
            return new QuorumCertificate(block.getHash(), block.getNexusBlockHeader().getHeight(),
                    (long) block.getNexusBlockHeader().getRoundIdentifier().getRoundNumber(), Bytes.wrap(aggregate), bitSet
            );
        }
...
//        ));
    ...
    private void sendBlockAnnounce(Commit msg) {
        getRoundState().setCurrentState(NexusMessage.BLOCK_ANNOUNCE);
...
//                finalState.getBlockTimer().cancelTimer();
    ...
    private <P extends NexusPayload, M extends BftMessage<P>> void actionOrBufferMessage(
            final M posMessage,
    ...
    private void setCurrentRound(final int roundNumber) {
        ...
        currentRound.get().setNexusBlockHeightManager(this);
