...
public class NexusBlockCreator {

  private static final Logger log = LogManager.getLogger(NexusBlockCreator.class);
  private final BlockCreator besuBlockCreator;
  private final NexusExtraDataCodec posExtraDataCodec;


  public NexusBlock createBlock(
          final long headerTimeStampSeconds, final NexusBlockHeader parentHeader, Address proposer) {
...
//    NexusBlockHeader header=NexusBlockHeader.builder()
//            .besuHeader(blockResult.getBlock().getHeader())
//            .roundIdentifier(round)
//            .proposer(proposer)
//            .build();
    return new NexusBlock(blockResult.getBlock(),round,proposer);
  }

  public NexusBlock createSealedBlock(
          final NexusBlock block, final int roundNumber, final Collection<SECPSignature> commitSeals, Address propser,
    ...
    final NexusBlockHeader initialHeader = block.getHeader();
    final NexusExtraData initialExtraData =
        posExtraDataCodec.decodeNexus(BlockUtil.toBesuBlockHeader(initialHeader));

    final NexusExtraData sealedExtraData =
        new NexusExtraData(
    ...
    final BlockHeader sealedHeader =
        BlockHeaderBuilder.fromHeader(BlockUtil.toBesuBlockHeader(initialHeader))
            .extraData(posExtraDataCodec.encodeNexusData(sealedExtraData))
    ...
    return new NexusBlock(sealedBesuBlock,block.getNexusBlockHeader());
  }
}
