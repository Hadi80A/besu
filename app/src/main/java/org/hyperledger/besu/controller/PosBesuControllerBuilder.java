...
public class PosBesuControllerBuilder extends BesuControllerBuilder {
    ...
    protected void prepForBuild() {
        ...
        // Use ECDSA codec for DSS compliance
    ...
    protected MiningCoordinator createMiningCoordinator(
        ...
        // Phase 4: Block Creation Factory using ECDSA (implied by nodeKey and codec)
        ...
        // Generic selector for P2P/Validator list management
        ...
        minedBlockObservers.subscribe(ethProtocolManager::blockMined);
        ...
        // Phase 1: Initialization - Build the State/Ledger of Stake
        ...
        // Phase 3: Proposer Selection - Initialize FTS Selector
        PosProposerSelector posProposerSelector = null;
        if (nodeSet.getNode(localAddress).isPresent()) {
            // Initialize the selector with the weighted NodeSet
    ...
    private static Bytes32 computeMappingSlotForAddress(final Address addr, final long mappingSlotIndex) {
        Bytes addrBytes = Bytes.wrap(addr.toArray());
        Bytes32 paddedKey = Bytes32.leftPad(addrBytes);
     ...
     * Builds the initial validator set (Stake Ledger) from Genesis ExtraData and World State.
     * Conforms to DSS by using strictly ECDSA keys.
     */
    private NodeSet createNodeSet(MutableWorldState worldState, MutableBlockchain blockchain) {
        ...
        BftExtraData posExtraData = codec.decodeRaw(genesisHeader.getExtraData());

        // Extract validator addresses and ECDSA public keys
        List<Address> validators = posExtraData.getValidators().stream().toList();

        // Removed PosExtraDataCodec.setPublicKeys(publicKeys) call here

        Address stakeManager = posConfig.getContractAddress();

        System.out.println("========== Validator Wallets (PoS / DSS) ==========");
        System.out.printf(
                "%-20s | %-42s | %-15s | %-15s%n",
        ...
        for (int index = 0; index < validators.size(); index++) {
            ...
            // Phase 1: Reading Stake from Contract State
            ...
            // Build node info
            StakeInfo stake = StakeInfo.builder().stakedAmount(stakeEth.longValue()).build();
            ...
            LOG.debug("Stake for {}: {}", id, node.getStakeInfo().getStakedAmount());
