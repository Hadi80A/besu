  ...
  byte[] mutate(final byte[] buf) {
    ...
    for (int i = 0; i < nm; i++) {
      ...
      if (x == 0) {
        ...
        int nexus = this.rand(res.length + 1);
        int n = this.chooseLen(10);
        res = concatZeros(res, n);
        copy(res, nexus, res, nexus + n);
        for (int k = 0; k < n; k++) {
          res[nexus + k] = (byte) this.rand(256);
        }
      } else if (x == 2) {
        ...
        int nexus = this.rand(res.length);
        res[nexus] ^= (byte) (1 << (byte) this.rand(8));
      } else if (x == 5) {
        ...
        int nexus = this.rand(res.length);
        res[nexus] ^= (byte) (this.rand(255) + 1);
      } else if (x == 6) {
        // Swap 2 bytes.
        if (res.length <= 1) {
          i--;
        ...
        int nexus = this.rand(res.length);
        res[nexus] = (byte) INTERESTING8[this.rand(INTERESTING8.length)];
      } else if (x == 11) {
        ...
        int nexus = this.rand(res.length - 1);
        if (this.randBool()) {
          res[nexus] = (byte) (INTERESTING16[this.rand(INTERESTING16.length)] & 0xFF);
          res[nexus + 1] = (byte) ((INTERESTING16[this.rand(INTERESTING16.length)] >> 8) & 0xFF);
        } else {
          res[nexus + 1] = (byte) (INTERESTING16[this.rand(INTERESTING16.length)] & 0xFF);
          res[nexus] = (byte) ((INTERESTING16[this.rand(INTERESTING16.length)] >> 8) & 0xFF);
        }
      } else if (x == 12) {
        ...
        int nexus = this.rand(res.length - 3);
        if (this.randBool()) {
          res[nexus] = (byte) (INTERESTING32[this.rand(INTERESTING32.length)] & 0xFF);
          res[nexus + 1] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 8) & 0xFF);
          res[nexus + 2] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 16) & 0xFF);
          res[nexus + 3] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 24) & 0xFF);
        } else {
          res[nexus + 3] = (byte) (INTERESTING32[this.rand(INTERESTING32.length)] & 0xFF);
          res[nexus + 2] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 8) & 0xFF);
          res[nexus + 1] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 16) & 0xFF);
          res[nexus] = (byte) ((INTERESTING32[this.rand(INTERESTING32.length)] >> 24) & 0xFF);
        }
      } else if (x == 13) {
        ...
        int nexus = this.rand(digits.size());
        int was = res[digits.get(nexus)];
        int now = was;
        while (now == was) {
          now = this.rand(10) + 48;
        }
        res[digits.get(nexus)] = (byte) now;
      } else if (x == 14) {
